export default function BatterCard({ batter, open, toggleOpen, onDelete, navigate }) {
  return (
    <div className="bg-white shadow-md rounded-xl p-5 hover:shadow-xl transition">
      <div className="flex justify-between items-center mb-2">
        <h3 className="font-semibold">
          {batter.user.name} ({batter.order_no}番 / {batter.position})
        </h3>
      ...
      <div className="grid grid-cols-3 gap-2 mb-2">
        <div>
          <p>打率</p>
          <span className="font-bold">{batter.batting_average}</span>
        </div>
        <div>
          <p>出塁率</p>
          <span className="font-bold">{batter.on_base_percentage}</span>
        </div>
        <div>
          <p>盗塁率</p>
          <span className="font-bold">{batter.steal_success_rate}</span>
        </div>
        <div>
          <p>長打率</p>
          <span className="font-bold">{batter.slugging}</span>
        </div>
        <div>
          <p>OPS</p>
          <span className="font-bold">{batter.ops}</span>
        </div>
      </div>
      ...
      <div className="text-right">
        <button
          onClick={toggleOpen}
          className="text-blue-600 text-sm hover:underline mt-3"
        >
          {open ? "▲ 詳細を閉じる" : "▼ 詳細を見る"}
        </button>
      </div>
