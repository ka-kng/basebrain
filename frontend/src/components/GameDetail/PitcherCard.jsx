export default function PitcherCard({ pitcher, open, toggleOpen, onDelete, navigate }) {
  return (
    <div className="bg-white shadow-md rounded-xl p-5 hover:shadow-xl transition">
      <div className="flex justify-between items-center mb-2">
        <h3 className="font-semibold">
          {pitcher.user.name} ({pitcher.formatted_innings}回)
        </h3>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div>
          <p>防御率</p>
          <span className="font-bold">{pitcher.era}</span>
        </div>
        <div>
          <p>奪三振率</p>
          <span className="font-bold">{pitcher.k9}</span>
        </div>
        <div>
          <p>与四死球率</p>
          <span className="font-bold">{pitcher.bb9}</span>
        </div>
      </div>
